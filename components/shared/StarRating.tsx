import React from 'react';

const Star: React.FC<{ filled?: boolean; half?: boolean }> = ({ filled = false, half = false }) => {
    const starPath = "M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z";
    
    if (half) {
        return (
            <div className="relative w-5 h-5">
                <svg className="w-5 h-5 text-slate-300" fill="currentColor" viewBox="0 0 20 20"><path d={starPath} /></svg>
                <div className="absolute top-0 left-0 w-1/2 h-full overflow-hidden">
                     <svg className="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20"><path d={starPath} /></svg>
                </div>
            </div>
        )
    }

    return (
        <svg className={`w-5 h-5 ${filled ? 'text-yellow-400' : 'text-slate-300'}`} fill="currentColor" viewBox="0 0 20 20">
            <path d={starPath} />
        </svg>
    );
};

interface StarRatingProps {
  rating: number;
}

const StarRating: React.FC<StarRatingProps> = ({ rating }) => {
  const fullStars = Math.floor(rating);
  const hasHalfStar = rating % 1 >= 0.5;
  const emptyStars = 10 - fullStars - (hasHalfStar ? 1 : 0);

  return (
    <div className="flex items-center space-x-2">
        <div className="flex">
            {Array.from({ length: fullStars }).map((_, i) => <Star key={`full-${i}`} filled />)}
            {hasHalfStar && <Star key="half" half />}
            {Array.from({ length: emptyStars }).map((_, i) => <Star key={`empty-${i}`} />)}
        </div>
        <span className="text-sm font-semibold text-slate-600">{rating > 0 ? `${rating.toFixed(1)}/10` : ''}</span>
    </div>
  );
};

export default StarRating;